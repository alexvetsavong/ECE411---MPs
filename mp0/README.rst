...
To create your git repository, go to `<https://edu.cs.illinois.edu/create-ghe-repo/ece411-fa20/>`_.
...
your netid)::
  ...
  $ git init
  $ git remote add origin git@github-dev.cs.illinois.edu:ece411-fa20/NETID.git
  $ git remote add release git@github-dev.cs.illinois.edu:ece411-fa20/_release.git
  $ git fetch release
...
In Quartus, use the **New Project Wizard** to create your MP0 project. Use your git repository's MP0 directory as the
project working directory. Name the project mp0. Create an empty project, and add ``mp0/hdl/mp0.sv`` to the project files. Under
...
Under the **Test bench and simulation files** section, add ``mp0/hvl/mp0_tb.sv`` to the testbench files.
...
You can also execute this Tcl script from the ModelSim shell. You can do this by running (from the terminal)::
    ...
    $ vsim -c
    > do simulation/modelsim/mp0_run_msim_rtl_verilog.do
...
stores a log of the output that ModelSim produces. Make a copy of this file in your mp0 directory by running::
...
Then push your changes to your remote repository. You are now done with this MP! If all goes well, the autograder
